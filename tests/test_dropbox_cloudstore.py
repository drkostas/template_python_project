...
class TestDropboxCloudStore(unittest.TestCase):
    ...
    def test_connect(self):
        ...
        try:
            ...
            self.logger.error('Error connecting with the correct credentials: %s', e)
            self.fail('Error connecting with the correct credentials')
        else:
            self.logger.info('Connected with the correct credentials successfully.')
        ...
        with self.assertRaises(BadInputError):
            ...
            self.logger.info("Loading Dropbox with wrong credentials failed successfully.")
    ...
    def _generate_random_filename_and_contents() -> Tuple[str, str]:
        ...
        contents = ''.join(random.choice(letters) for i in range(20))
    ...
    @classmethod
    def setUp(cls) -> None:
        cls.file_name, contents = cls._generate_random_filename_and_contents()
        with open(os.path.join(cls.test_data_path, cls.file_name), 'a') as f:
            f.write(contents)

    @classmethod
    def tearDown(cls) -> None:
        os.remove(os.path.join(cls.test_data_path, cls.file_name))
    ...
    def setUpClass(cls):
        ...
        if "DROPBOX_API_KEY" not in os.environ:
            cls.logger.error('DROPBOX_API_KEY env variable is not set!')
